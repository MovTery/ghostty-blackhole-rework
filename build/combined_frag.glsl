...
const float STAR_GAIN     = 1.0000; // lensed starfield brightness around the hole (0 = off)
...
const float DISK_GAIN     = 0.0; // disk emission brightness
...
// bumps iTime on ANY cursor change *including color* and
...
float tokenLevel() {
    ...
    return mix(prev, cur, smoothstep(0.0, T, iTime - iTime));
}
...
// is the last five minutes of every hour. Independently, iTime
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (SIZE_MODE == MODE_POMODORO) {
        ...
        float idle = max(0.0, iTime - iTime);
        ...
        if (lvl < 0.0) { fragColor = vec4(sky(uv), 1.0); return; }
